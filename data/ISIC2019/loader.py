...
def merge_labels_and_metadata (df_labels, df_cli_feat, save_path=None):
    ...
    if save_path is not None:
        df_labels_cli_feat.to_csv(os.path.join(save_path), index=False)

    return df_labels_cli_feat
    
    

def save_imgs_to_folder(imgs_path, imgs_save_dir, imgs_base_dir = 'ISIC_2019_Training_Input/'):
    for img_path in imgs_path:
        img_path_sr = imgs_base_dir + img_path
        shutil.copy(img_path_sr, imgs_save_dir)

def split_train_test_imgs_csv (data_csv, save_path_train_csv=None, save_path_test_csv=None, \
                                save_imgs_test_dir=None, save_imgs_train_dir=None, \
                                imgs_base_dir=None, tr=0.80, te=0.20, seed_number=2021, img_format='.jpg'):
    # Loading the data_csv
    if isinstance(data_csv, str):
        data_csv = pd.read_csv(data_csv)
    ...
    # Getting train/test sequences
    N = len(data_csv)
    n_test = int(round(te * N))
    n_train = N - n_test
    
    # get image ID
    imgs_path = list(x+img_format for x in data_csv['image'])
    imgs_path_test = imgs_path[:n_test] 
    imgs_path_train = imgs_path[n_test:]
    
    # save train/test images
    save_imgs_to_folder(imgs_path_test, save_imgs_test_dir, imgs_base_dir=imgs_base_dir)
    save_imgs_to_folder(imgs_path_train, save_imgs_train_dir, imgs_base_dir=imgs_base_dir)
    
    # save train/test csv files
    if save_path_test_csv is not None:
        data_csv.loc[0:n_test-1].to_csv(save_path_test_csv, index=False)
    
    if save_path_train_csv is not None:
        data_csv.loc[n_test:].to_csv(save_path_train_csv, index=False)    
